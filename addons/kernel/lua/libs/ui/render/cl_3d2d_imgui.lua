...
AMB.UI.Render.Imgui = AMB.UI.Render.Imgui or {}

AMB.UI.Render.Imgui.skin = {
...
local is_dev = false
function AMB.UI.Render.Imgui.IsDeveloperMode()
	return is_dev
end

local _devMode = false -- cached local variable updated once in a while

function AMB.UI.Render.Imgui.Hook(name, id, callback)
...
AMB.UI.Render.Imgui.Hook("PreRender", "Input", function()
...
function AMB.UI.Render.Imgui.Start3D2D(pos, angles, scale, distanceHide, distanceFadeStart)
	...
	if gState.rendering == true then
		...
		AMB.Debug( function() MsgC( C.ABS_RED, '[DEBUG] ', C.ABS_WHITE, ' Imgui Lib. stopped the render!\n' ) end )

		gState.shutdown = true
		return false
	end

	_devMode = AMB.UI.Render.Imgui.IsDeveloperMode()
...
function AMB.UI.Render.Imgui.Entity3D2D(ent, lpos, lang, scale, ...)
	gState.entity = ent
...
function AMB.UI.Render.Imgui.ExpandRenderBoundsFromRect(x, y, w, h)
...
function AMB.UI.Render.Imgui.End3D2D()
	if gState then
		if _devMode then
...
function AMB.UI.Render.Imgui.CursorPos()
	local mx, my = gState.mx, gState.my
	return mx, my
end

function AMB.UI.Render.Imgui.IsHovering(x, y, w, h)
	local mx, my = gState.mx, gState.my
	return mx and my and mx >= x and mx <= (x + w) and my >= y and my <= (y + h)
end
function AMB.UI.Render.Imgui.IsPressing()
	return shouldAcceptInput() and gState.pressing
end
function AMB.UI.Render.Imgui.IsPressed()
...
function AMB.UI.Render.Imgui.SetFont(font, defaultSize)
	-- special font
...
function AMB.UI.Render.Imgui.DrawButton(x, y, w, h, borderWidth, borderClr, hoverClr, pressColor)
	local bw = borderWidth or 1

	local bgColor = AMB.UI.Render.Imgui.IsHovering(x, y, w, h) and AMB.UI.Render.Imgui.skin.backgroundHover or AMB.UI.Render.Imgui.skin.background
	local borderColor =
		((AMB.UI.Render.Imgui.IsPressing() and AMB.UI.Render.Imgui.IsHovering(x, y, w, h)) and (pressColor or AMB.UI.Render.Imgui.skin.borderPress))
		or (AMB.UI.Render.Imgui.IsHovering(x, y, w, h) and (hoverClr or AMB.UI.Render.Imgui.skin.borderHover))
		or (borderClr or AMB.UI.Render.AMB.UI.Render.Imgui.skin.border)
	...
	return shouldAcceptInput() and AMB.UI.Render.Imgui.IsHovering(x, y, w, h) and gState.pressed
end

function AMB.UI.Render.Imgui.DrawCursor(x, y, w, h)
	local fgColor = AMB.UI.Render.Imgui.IsPressing() and AMB.UI.Render.Imgui.skin.foregroundPress or AMB.UI.Render.Imgui.skin.foreground
	local mx, my = gState.mx, gState.my
...
function AMB.UI.Render.Imgui.DrawTextButton(text, font, x, y, w, h, borderWidth, color, hoverClr, pressColor)
	local fgColor =
		((AMB.UI.Render.Imgui.IsPressing() and AMB.UI.Render.Imgui.IsHovering(x, y, w, h)) and (pressColor or AMB.UI.Render.Imgui.skin.foregroundPress))
		or (AMB.UI.Render.Imgui.IsHovering(x, y, w, h) and (hoverClr or AMB.UI.Render.Imgui.skin.foregroundHover))
		or (color or AMB.UI.Render.Imgui.skin.foreground)

	local clicked = AMB.UI.Render.Imgui.DrawButton(x, y, w, h, borderWidth, color, hoverClr, pressColor)

	font = AMB.UI.Render.Imgui.SetFont(font, math.floor(h * 0.618))
	draw.SimpleText(text, font, x + w / 2, y + h / 2, fgColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

	return clicked
end

return AMB.UI.Render.Imgui
